...
{
    int maxEpisodeCount = shows.Max(s => s.EpisodeCount);
    return shows.FirstOrDefault(s => s.EpisodeCount == maxEpisodeCount);
}
...
{
   return string.Join(", ", shows.SkipLast(1).Select(s => s.Name)) +
           (shows.Count > 1 ? $" and {shows.Last().Name}" : "");
}
